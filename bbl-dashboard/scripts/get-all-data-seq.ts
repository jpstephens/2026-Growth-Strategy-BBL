/**
 * Script to get ALL historical data from Alvys - Sequential to avoid rate limits
 */
...
async function alvysFetch<T>(endpoint: string, body?: object): Promise<T> {
...
async function main() {
  console.log('=== Getting ALL Historical Data from Alvys (Sequential) ===\n');

  try {
    // Get token first
    await getAccessToken();

    // Query each status one at a time - valid statuses from API
    const statuses = ['Invoiced', 'Delivered', 'Dispatched', 'Covered', 'Queued', 'In Transit', 'Completed', 'Paid', 'Carrier Paid', 'Released'];
    const allLoads: any[] = [];

    for (const status of statuses) {
      console.log(`Fetching ${status} loads...`);
      try {
        // Need to paginate - max 500 per page
        let page = 0;
        let hasMore = true;
        while (hasMore) {
          const response = await alvysFetch<{ Items: any[], Total: number, Page: number, PageSize: number }>('/loads/search', {
            PageSize: 500,
            Page: page,
            Status: [status],
          });
          const loads = response.Items || [];
          console.log(`  Page ${page}: ${loads.length} loads (Total: ${response.Total})`);
          allLoads.push(...loads);

          if (loads.length < 500 || allLoads.length >= response.Total) {
            hasMore = false;
          } else {
            page++;
          }
    ...
    if (uniqueLoads.length === 0) {
      console.log('\nNo loads found. Checking API response format...');
      // Try a simple test
      const testResponse = await alvysFetch<any>('/loads/search', {
        PageSize: 10,
        Status: ['Invoiced'],
      });
    ...
    for (let i = 0; i < loadNumbers.length; i += batchSize) {
      ...
      console.log(`  Batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(loadNumbers.length/batchSize)}...`);
      try {
        const response = await alvysFetch<{ Items: any[] }>('/trips/search', {
          PageSize: 500,
          LoadNumbers: batch,
        });
    ...
    console.log('\n\n=== COPY THIS TO src/data/currentState.ts ===\n');
...
export const topCustomers = [`);
    ...
    assessment: 'Data from Alvys API',
    whyIdeal: 'Verified ${new Date().toISOString().split('T')[0]}',
  },`);
    }
    console.log(`]`);
